...
#' @param sample_label Character, legend label.
...
function (df_samples, 
  ...
  expr_median = df_samples %>% 
    ...
    dplyr::summarise_at(protein_names, median) %>% as.data.frame
  ...
  expr_median %<>% dplyr::bind_cols(tibble(MDS1 = mds_res$points[,1], MDS2 = mds_res$points[, 2]))
  ...
  expr_cor = stats::cor(expr_median[, protein_selection], expr_median[, c("MDS1", "MDS2")]) %>% tibble::as_tibble
  expr_cor %<>% tibble::add_column(protein_selection)
  expr_cor %<>% tibble::add_column(x0 = rep(0, nrow(expr_cor)))
  expr_cor %<>% tibble::add_column(y0 = rep(0, nrow(expr_cor)))
  # Do not display protein which are < cor_threshold of both MSD1/2
  expr_cor_filtered <- expr_cor %>% 
    dplyr::filter(abs(MDS1) > cor_threshold & abs(MDS2) > cor_threshold)
  # Draw the circle
  ...
  circle_plot <- ggplot() + 
    ...
    geom_segment(data = expr_cor_filtered, aes(x = x0, y = y0, xend = MDS1, yend = MDS2), colour = "gray65") + 
    geom_text_repel(data = expr_cor_filtered, aes(x = MDS1, y = MDS2, label = protein_selection)) + 
  ...
  # If there is label specify for the legeng, add it to the MDS plot
