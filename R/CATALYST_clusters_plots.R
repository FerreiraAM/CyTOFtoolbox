...
extract_SCE_info <- function(sce, metacluster, dr = "UMAP"){
  # Extract DR info 
  dr_values <- SingleCellExperiment::reducedDim(sce, dr)
  ## Check colnames
  if (any(colnames(dr_values) != c("X1", "X2"))) {
  ...
  sce_metacluster_ids <- as.data.frame(CATALYST::cluster_ids(sce, metacluster))
...
plot_metaclusters <- function(df, metacluster, dr = "UMAP", 
  ...
  ggplot(df, aes_string(x = "X1", y = "X2")) +
    ...
    scale_color_manual(values = catalyst_col, name = metacluster) +
