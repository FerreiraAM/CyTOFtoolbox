...
to stabilize the mean–variance relationship. Then, one Linear Model is fitted for each cluster.

```{r DA_tests_voom}
# Design & constrast matrix are the same as for the diffcyt-DA-edgeR test
## design_matrix_Bodenmiller
## contrast_Bodenmiller
# Test with voom
## Non-parametric test
...
The `plotDiffHeatmap` function is modified to get colors more different for the samples
and conditions, as well as the heat color scale goes from grey to red. Additionally,
there is the possibility to hide the sample IDs with the `show_sample_ID` parameter
...
# There is a typo in the CATALYST package on the colnames of the results
# The 2 following lines fix it for now
col_idx <- grep("logFC", names(rowData(res_DA_Bodenmiller_voom$res)))
names(rowData(res_DA_Bodenmiller_voom$res))[col_idx] <- "log_FC"
# Plot
plotDAheatmap(x = sce_Bodenmiller,
