...
- *type* markers are gating/phenotype markers. They are commonly used to perfors 
Differential Abundance (DA) tests;
- *state* markers are functional markers. They are commonly used to perfors 
...
the FCS files. If it is not the case, we add to the missing marker to this list.
...
The last step is to create a `SingleCellExperiment` containing the FCS file values,
...
The dimension reduction is computed with UMAP.
...
and cannot be saved as a PDF), the following code can be used and adpated:
